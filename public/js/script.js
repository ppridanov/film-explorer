...
function getGenres(data) {
  fetch(`https://api.themoviedb.org/3/genre/movie/list?api_key=${data.apiKey}&language=en-US`, {
    method: 'GET',
    cors: 'no-cors'
  })
  .then((res) => {
...
function getFilms(data) {
  fetch(`https://api.themoviedb.org/3/movie/popular?api_key=${data.apiKey}&language=en-US&page=1`, {
    method: 'GET',
    cors: 'no-cors'
  })
  .then((res) => {
  ...
  .then((data) => {
    ...
    data.results.forEach((el) => {
      ...
      img.setAttribute('src', `https://image.tmdb.org/t/p/w185${el.poster_path}`);
      div.appendChild(img);
